...
public class ProductSizesController : ControllerBase
{
    ...
    {
        var productSizes = await _mediator.Send(new GetProductSizesQuery(productSizeParams));
    ...
    {
        var productSize = await _mediator.Send(new GetProductSizeByIdQuery(id));
    ...
    {
        var productSizes = await _mediator.Send(new GetProductSizesByProductIdQuery(productId));
        return Ok(productSizes);
    }
    ...
    {
        var productSize = await _mediator.Send(new CreateProductSizeCommand(productSizeDto));
    ...
    {
        var productSize = await _mediator.Send(new UpdateProductSizeCommand(id, productSizeDto));
        return Ok(productSize);
    }
    ...
    {
        var productSize = await _mediator.Send(new DeleteProductSizeCommand(id));
        return Ok(productSize);
    }
}